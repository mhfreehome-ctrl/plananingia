...
const MAX_VIS_H = 660  // hauteur max visible avant scroll vertical

export default function STWorkload() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [u, l] = await Promise.all([
        api.users.list().then((arr: any[]) => arr.find((x: any) => x.id === id) || null),
        api.users.lots(id),
      ])
      setUser(u)
      setLots(l)
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          <Link to="/users" className="text-sm text-gray-400 hover:text-gray-600">← Utilisateurs</Link>
          <h1 className="text-xl font-bold text-gray-900">Charge — {user.first_name} {user.last_name}</h1>
        </div>
        <div className="card card-body text-center text-gray-400 py-12">
          Aucun lot avec dates planifiées assigné à {isEmployee ? 'ce salarié' : 'ce sous-traitant'}.
        </div>
      </div>
    )
  }
  ...
  // ── Rows (liste plate : header chantier + lots) ──────────────
  type Row =
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">
              Charge — {user.first_name} {user.last_name}
            </h1>
            <p className="text-sm text-gray-400">
              {isEmployee ? '👷 Salarié' : '🏢 Sous-traitant'}
      ...
      {conflictCount > 0 && (
        <div className="bg-orange-50 border border-orange-200 text-orange-800 text-sm px-4 py-3 rounded-lg">
          ⚠ Des lots se chevauchent temporellement (surlignés en orange). Vérifiez la disponibilité.
        </div>
      )}
                ...
                if (row.type === 'project') {
                  ...
                  return (
                    <div key={`lot-${lot.id}`} style={{ height: ROW_H }}
                      ...
                      <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: lot.color || '#6B7280' }} />
                      <div className="min-w-0 flex-1">
          ...
          >
            <div style={{ width: CHART_W, minWidth: '100%' }}>
              ...
              <div style={{ position: 'relative', height: totalH }}>
                ...
                {rows.map((row, i) => {
                  if (row.type === 'project') {
                    // Barre span du chantier (transparente)
                    ...
                    const barColor = isConflict ? '#f97316' : (lot.color || '#6B7280')
                    return (
                      <div key={`lbar-${lot.id}`}
      ...
      <div className="flex items-center gap-5 text-xs text-gray-500 flex-wrap">
        <span className="flex items-center gap-1.5">
          <span style={{ display: 'inline-block', width: 16, height: 12, borderRadius: 3, backgroundColor: '#6366f1', opacity: 0.12, border: '1.5px solid #6366f1' }} />
          Span chantier
        </span>
        <span className="flex items-center gap-1.5">
